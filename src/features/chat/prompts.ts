...
A form often constrains what a cell may contain through text that is NOT inside the cell: a legend or footnote near the table (often a line led by \`*\`, \`※\`, \`주)\`, or "범례/기준/수준:"), an enumerated scale written as arrow- or slash-separated options, a parenthetical option list in a header cell, or an instruction paragraph above the table. \`getEmptyFormFields\` returns only the cells — it does NOT carry these. \`getDocumentSummary\` surfaces the ones it can detect under a \`[Value vocabularies …]\` block — read that block, and for anything it might miss, read the form's surrounding text (\`getTextRange\` / \`searchAllText\` on the paragraphs just before/after the table from \`tableInventory\`) and extract any such constraint.
