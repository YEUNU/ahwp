...
export const webFetch = defineTool<'webFetch', AhwpToolArgs['webFetch']>({
  name: 'webFetch',
  description:
    'Fetch a URL (http or https only) and return the page content as plain text. For HTML pages, the runtime uses **Mozilla Readability** (Firefox Reader Mode engine) to extract the article body — nav/sidebar/footer/ads are stripped, and metadata (title, byline, excerpt, siteName) is returned alongside. For non-article pages (search results, SPA shells) it falls back to regex tag-stripping. The `extractionMethod` field in the response tells you which path was used. Capped at maxBytes (default 32KB) — large pages are trimmed. Pass `prompt` to keep your intent visible in the response. 30-second timeout. Use for: (a) "이 페이지 내용 요약해줘" with a URL, (b) verifying specific claims against an external source. Do NOT use for: searching the web (use webSearch instead) or fetching paginated content (call repeatedly with different URLs).',
  inputSchema: {
    type: 'object',
    properties: {
      url: {
