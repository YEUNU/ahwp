...
**증상**: `@rhwp/core` 0.7.9는 표용 `getTableBBox(sec, parentPara, controlIdx)` → `{pageIndex, x, y, width, height}` 만 통합 bbox API로 publish. 이미지·도형 컨트롤은 `getShapeProperties(sec, parentPara, controlIdx)` 등 종류별 별도 호출이 있지만 페이지 좌표 + zoom 단위로 정규화된 bbox 일관 API는 없음. 결과: 0.2.74의 control highlight가 표만 동작, 이미지·도형 위 드래그 시 객체 자체는 시각적으로 강조되지 않음

**검증**: `node_modules/@rhwp/core/rhwp.d.ts` `getRect|BBox|Bounds` grep
