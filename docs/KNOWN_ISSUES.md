...
- 수식 렌더링: 차이 가능성 **중간**
- 복잡한 표·병합 셀: 미묘한 차이 가능 **중간 → 높음** (다중 행/열 병합이 있는 양식 표에서 column width 추정이 어긋나 우측 narrow column 의 텍스트가 잘리거나 행 높이가 불균형하게 보임. lib 가 Canvas 를 직접 그려 우리에게 넘기므로 우리 쪽 fix 불가. chunks 1~106 시점엔 SVG 경로의 `<text><title>` injection 으로 hover tooltip 우회를 했으나 chunk 107 의 Canvas-only 전환 으로 동일 우회 불가 — 후속 작업으로 `getPageLayerTree` text op bbox + transparent overlay tooltip 검토 가능)
