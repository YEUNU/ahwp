...
| 항목        | 상태                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Phase       | **Phase 1~5 대부분 완료 (chunks 1~95 + 95.1)** — Phase 3 MVP Agent 모드 + 라이브 OpenAI / NVIDIA NIM / Google Gemini / Custom OpenAI-호환. tool 카탈로그 **54개** (write 45 + read 9). docId-aware Agent dispatch (chunks 50/59), Diff Viewer (Q5), 자체 호스팅 crash reporter (chunk 63), i18next ko/en 인프라 + 한컴 매뉴얼 명칭 호버 툴팁 (chunks 89~95). 잔여: chunk 42 Anthropic 어댑터 (API 키 대기), chunks 33/36/87 (`@rhwp/core` 0.8 대기), Phase 4 macOS notarization / Windows 서명, Phase 5 베타 채널 |
| 버전        | `0.3.31`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| 단위 테스트 | ✅ `App.test.tsx`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| e2e         | 365 케이스 통과 (chunks 80~84 메이저 일괄 업그레이드 후 354 → 365). 1 케이스 알려진 회귀: `studio-paraformat` save→reopen alignment (lib 0.8 대기)                                                                                                                                                                                                                                                                                                                                                                |
| 메이저      | Electron 41 · React 19 · Vite 8 · TypeScript 6 · Tailwind 4 (CSS-first @import + tw-animate-css) · react-resizable-panels v2 (v4 layout 회귀로 환원) · sandbox=true · contextIsolation=true                                                                                                                                                                                                                                                                                                                       |
...
현재 `0.3.31`, chunks 1~95 + 95.1 완료. Phase 1~5 사용자-facing 기능은 거의 다 들어갔고, 남은 항목은 외부 의존 / 배포 서명 / 베타 채널 정도.

| 영역                | 항목                                                                                                   | 상태                  |
| ------------------- | ------------------------------------------------------------------------------------------------------ | --------------------- |
| Phase 2-B / Phase 3 | Anthropic 어댑터 + tool_use 통합 (chunk 42)                                                            | API 키 결정 대기      |
| Phase 2-F           | chunk 33 도형 라인/곡선/그룹, chunk 36 스타일 char/para shape 캡처                                     | `@rhwp/core` 0.8 대기 |
| Phase 4             | macOS notarization, Windows 코드 사이닝                                                                | 메인테이너 결정 대기  |
| Phase 5             | 베타 사용자 피드백 채널 (GitHub Discussions)                                                           | 미개설                |
| i18n 점진           | Settings / Folder / Chat 의 잔여 hardcoded 텍스트 ko→en 마이그레이션                                   | 후속 청크             |
| lib                 | chunk 87 표 column-width 후처리 (`@rhwp/core` 0.8.x column width 추정 fix 대기), L-001 HWPX 라운드트립 | 라이브러리 대기       |

라이브러리 의존 정리: KNOWN_ISSUES L-001 (HWPX 라운드트립 → save canonical=HWP) / L-004 (표 column-width 추정) / L-006 (cell 배경색 setter 부재) / chunk 33 / chunk 36 — 모두 `@rhwp/core` 0.8 대기.

이력은 ROADMAP.md Phase 5 섹션의 chunk 단위 체크박스로 옮겨 관리. 신규 청크는 거기 추가.
