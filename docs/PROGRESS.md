...
| 항목     | 상태                                                                                                                                                                                                          |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Phase    | **1-D 완료 직전** (Studio 마이그레이션 청크 1~6 완료 — 텍스트 편집 + 키보드/마우스/IME + 시각 커서 + 툴바 + iframe 제거)                                                                                      |
...
- 선택 모델 (Shift+Arrow / 마우스 드래그 → range) 후 range-format
...
- `file:new` (`createBlankDocument` 기반 빈 시드 — 시작 화면용)
- 시작 화면(Welcome view) — 현재는 ipc:ping 데모
- `extractThumbnail` / `exportHwpVerify` — Phase 4(패키징) 직전까지 보류

### Phase 1-D 후속 (선택 모델 + 추가 서식)

- 선택 모델 (Shift+Arrow / 마우스 드래그 → range)
- range 기반 applyCharFormat (선택 영역만 toggle)
