...
- `file:new` — `createBlankDocument`은 인스턴스 메서드라 빈 시드 HWPX가 필요. 옵션:
  1. 사용자 첫 .hwpx 저장 시 캐시해 이후 시드로 사용 (lazy)
  2. 최소 HWPX 스켈레톤을 코드로 생성 (zip + 정해진 OWPML XML들)
- `extractThumbnail` 활용 — FileList 좌측 패널 미리보기 (visible 폴리시)
- `exportHwpVerify` 활용 — 저장 시 검증 결과를 사용자에게 표시
- `@rhwp/core` 직접 viewer 통합 — `@rhwp/editor` iframe 의존 제거 (큰 결정)
