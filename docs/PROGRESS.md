...
| 항목        | 상태                                                                                                                                                                                                                                                                                                                                          |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Phase       | **Phase 6 완료 + L-004 follow-up (chunks 100~107)** — rhwp-studio view 계층 정합 완료. Canvas + 3-tier overlay 가 유일한 render path. behind/front floating image (워터마크 / 도장) 정확히 layered. L-004 narrow-column tooltip 도 Canvas mode 에서 복구 (`getPageTextLayout` per-run transparent overlay). 다음: Anthropic API 키 (chunk 42) |
| 버전        | `0.4.1`                                                                                                                                                                                                                                                                                                                                       |
...
### Phase 6 — rhwp-studio view 계층 정합 (2026-05-06) ✅
...
| 103b  | dual-mode re-render hooks (useDebugSurface + useUndoHistory selector → 'svg, canvas')                                                                                                        | ✅   |
| 104   | Phase 6.4 — behind/front overlay (`page-layer-tree.ts` 신설 + `parsePageLayerTree` + `applyOverlayLayers` + 효과 CSS + 워터마크 multiply + 6 단위 테스트)                                    | ✅   |
| 105   | Phase 6.5 — find / selection / changed-paragraph 모두 이미 DOM overlay → 변경 0. 회귀 가드 e2e 신규                                                                                          | ✅   |
