...
// SIM_GROUNDED=1 → faithful fill: write ONLY what the user's info
// (이즈파크/다빈치렌스/예지보전) actually grounds, leaving every ungrounded cell
// (numbers, 금액, KPI 목표치, 진척도, 정확도, maturity levels, markers) blank —
// the values a real agent would either omit or ASK the user for. The default
// (unset) over-fills with fabricated figures purely to exercise cell mechanics.
const GROUNDED = process.env.SIM_GROUNDED === '1';
...
// TABLE #2 — 1.1 구축 목표 (p=10). The objective IS grounded in the topic
// ("AI 예지보전"); keep it concise so it fits the fixed-height cell.
replace(
  10,
  0,
  4, // italic instruction placeholder
  GROUNDED
    ? 'AI 기반 설비 예지보전 시스템 구축'
    : '렌즈 제조 핵심설비(사출성형기·연마기·코팅장비)에 진동·전류·온도 센서를 부착하고 AI 예지보전 모델로 고장을 사전 예측하여 비계획 정지와 품질불량을 최소화',
);
// 기존/목표 maturity level (기초/중간2) is a judgment the user never gave →
// grounded mode leaves it blank (would ask the user instead).
if (!GROUNDED) {
  fill(10, 0, 5, '기초'); // 기존수준
...
} // end if (!GROUNDED) — everything above (1.3/1.4/1.5/2.2/AI성능 specifics)
// is ungrounded for this prompt; grounded mode fills only header + 1.1 objective.
