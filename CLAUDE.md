...
ahwp — Electron + React desktop app for viewing/editing Korean HWP/HWPX documents with AI assistance. **0.7.x** (current `0.7.50` — mixed-format workspace + auto-updater; 워크스페이스에 PDF/DOCX/Excel/TXT/MD/CSV/JSON/XML/HTML 등 혼합. 편집은 여전히 HWP/HWPX 만; 그 외는 AI read-only 컨텍스트 (실제 binary 라이브러리 round-trip 검증됨) + 클릭 시 OS 기본 앱 위임. Search 패널 / family 별 트리 아이콘 / per-family size cap. 새 버전 출시 시 TitleBar 아래 inline banner: "지금 받기 → 재시작해서 설치"). 라이브 provider: OpenAI / Google Gemini / custom OpenAI-호환 (Ollama/vLLM/LM Studio/on-prem; NIM 셀프호스트도 custom 슬롯). NIM 전용 어댑터는 0.6.18 에서 제거 (vision 부재로 form-fill 시각 검증 workflow 와 비호환). Anthropic 어댑터만 키 결정 대기.
...
npm run e2e          # vite build + Playwright Electron e2e (~140 cases, 4 workers / retries=1)
...
- `src/` — Renderer (React + Vite). Sandboxed: `nodeIntegration: false`, `contextIsolation: true`, `sandbox: true`. The renderer **cannot** import from `electron/` and has no Node access — `process` is undefined here (don't reference it; pass platform info from main if needed). Only `preload.ts`-exposed API is visible via `contextBridge`. Subdirs: `src/app/` (AppShell, TitleBar, WelcomePane, ThemeProvider, ThemeToggle, TabBar), `src/features/{files,rhwp-studio,chat,settings,cmdk,updater}/` (per-pane components — `FolderTree`, `RhwpEditor` (vendored rhwp-studio iframe host), `ChatPanel`, `SettingsDialog`, `CommandPalette` + `ShortcutsDialog`), `src/components/ui/` (shadcn primitives — Button, Dialog, Input), `src/lib/{utils.ts, rhwp-core/}`.
...
- HWP/HWPX content manipulation is split by process. In **MAIN**, save-side normalization + the magic-byte format sniff defer to `@rhwp/core` directly (its own `@rhwp/core` instance in `electron/hwp/converter.ts`). In the **RENDERER**, the live edit IR lives inside the `rhwp-studio` iframe (its `WasmBridge`), driven via `BridgeIrHelper` (`src/features/rhwp-studio/bridge-ir-helper.ts`); `src/lib/rhwp-core/` (init.ts) only pre-inits the WASM and installs the `measureTextWidth` callback.
...
- **Save canonical = HWP/CFB** (provisional). `@rhwp/core` v0.7.x's HWPX round-trip drops embedded image references (KNOWN_ISSUES L-001), so `file:save` writes HWP and auto-routes `.hwpx` paths to `.hwp`. Read path passes input bytes through unchanged. Will revert to HWPX once the lib fixes the round-trip — **L-001 still reproduces on v0.7.14** (exportHwpx round-trip이 임베드 이미지/페이지 수를 떨굼; exportHwp 는 무손실, 직접 검증). Currently on v0.7.14 (`searchAllText` adopted in `useFindReplace` — 0.4.29). 0.7.14 의 5개 메서드 시그니처 변경(`copyControl`/`exportControlHtml`/`getControlImageData`/`getControlImageMime`/`insertPicture` 에 `cell_path_json` 추가)은 vendored `rhwp-studio` 의 `WasmBridge` 래퍼에서 `''`(본문) 주입으로 흡수 — 호출부 무변경. New native APIs not yet wired: `getHeaderFooterPictureProperties` / `setHeaderFooterPictureProperties` (Task #825 머리말/꼬리말 안 그림 속성), `insertNewNumber` (쪽 > 새 번호 컨트롤), `getPageOverlayImages`, `searchText` (single-match forward/backward navigation), `getCanvasKitReplayPlan` (진단); **0.7.14 미연결** — 각주/미주 내부 편집(`applyParaFormatInFootnote` / `getNoteEditInfo` / `get`·`setNoteEquationProperties` / `getCursorRectInNote` / `getSelectionRectsInFootnote`), 셀-경로 그림·도형 **속성** family (`get`·`setCellPicturePropertiesByPath` / `get`·`setCellShapePropertiesByPath` / `deleteCellPictureControlByPath` / `pasteHtmlInCellByPath` / `pasteInternalInCellByPath`), `getExternalImageReferences` + `injectExternalImageByKey` (HWP3/외부 파일경로 이미지 주입). **0.7.14 연결됨(0.7.51~54)** — `get`·`setPageBorderFill`(쪽 테두리/배경), `insertEndnote`, `insertPicture` 의 `cellPath`(표 셀 이미지 — KNOWN_ISSUES L-007 실해결), `get`·`applyEndnoteShape`(미주 번호형식).
- **Prefer native rhwp over hand-rolled wrappers** — when the lib provides an API, call it directly. Don't re-implement scan loops / caches / parsing on top. Example: 0.4.29 dropped a paragraph-text cache + `indexOf` loop in `runFindSearch` once 0.7.12 shipped `searchAllText`.
...
- **E2E infrastructure** — Playwright + Electron, `tests/e2e/launch.ts` mints a fresh `--user-data-dir` per spec file. `playwright.config.ts` runs 4 workers locally / 2 in CI with `retries: 1` (parallel race noise; a true bug fails twice). `AHWP_E2E_FAKE_AI=1` swaps ALL provider adapters for `electron/ai/providers/fake.ts` — its ECHO/ERROR/SLOW prompt-encoded scripts cover deterministic streaming + abort cases. `examples/` is **tracked in git** — the .hwp fixtures (~5MB total) ship with the repo so CI and new clones don't auto-skip.
- **`__rhwpDebug` window surface** — the `rhwp-studio` editor iframe host (`src/features/rhwp-studio/debug-surface.ts`) exposes a debug entry on `window.__rhwpDebug` for e2e (insertText, applyCharFormat, openFind, openReplace, replaceAll, etc.). Only the active tab claims the surface (other viewers stay mounted but skip the global). When adding a new IR-mutating feature, expose it on `__rhwpDebug` _and_ add an e2e in lockstep — that's how chunks 1~7 stayed safe.
