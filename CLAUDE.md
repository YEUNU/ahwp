...
ahwp — Electron + React desktop app for viewing/editing Korean HWP/HWPX documents with AI assistance. **Phase 1~5 대부분 완료** (current `0.3.31`, chunks 1~95): full editor with visual-line caret nav / shift+click selection / drag auto-scroll / ⌘A IR-scoped, table cell editing v4 (cell-block TSV copy/paste) + table props + cell props + cell style + table formula evaluator, multi-line headers/footers with odd/even/both templates, image insert + picture props, control clipboard (⌘⇧C/V), HTML export, **Manual + Agent AI 모드** (chunks 37~41 Phase 3 MVP — provider-native tool calling, `ahwp-tools` / `ahwp-patches` 카탈로그 54 tools (write 45 + read 9), turn cap 10, 묶음 undo, "되돌리기" 토스트, multi-paragraph 발췌 + 멀티 문서 + docId-aware Agent dispatch chunks 50/59, Diff Viewer accept/reject), VS Code-style folder tree with chokidar + cross-tab external-change detection, browser-style tabs (pin + drag + context menu), command palette (⌘K) + shortcut cheatsheet + status bar counters, BYOK Settings 4탭 (일반 / AI 공급자 / 단축키 / 정보) with model dropdown auto-fetched from provider `/v1/models` + 24h cache + 시작 시 모든 키-등록 provider 병렬 pre-fetch, 자체 호스팅 crash reporter (chunk 63), i18next ko/en 인프라 (chunks 89/93, 점진 마이그레이션), 한컴 한글 매뉴얼 명칭 호버 툴팁 + 플랫폼별 단축키 표기 (chunks 90~95). 라이브 provider: OpenAI / NVIDIA NIM / Google Gemini (chunk 43) / custom OpenAI-호환 (chunk 44 — Ollama / vLLM / LM Studio / on-prem). Anthropic 어댑터만 키 결정 대기. See `docs/PROGRESS.md` for the up-to-date phase status — never assume features named in the README are implemented yet.
