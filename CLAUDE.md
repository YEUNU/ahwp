...
ahwp — Electron + React desktop app for viewing/editing Korean HWP/HWPX documents with AI assistance. **Phase 7 완료, 0.5.x stabilization** (current `0.5.2` — 24 broken AI tools 복원 + Diff Viewer 자동 적용 회귀 fix). 라이브 provider: OpenAI / NVIDIA NIM / Google Gemini / custom OpenAI-호환 (Ollama/vLLM/LM Studio/on-prem). Anthropic 어댑터만 키 결정 대기.
