...
[edwardkim/rhwp](https://github.com/edwardkim/rhwp)의 한글 파일 파서·렌더러 위에 Electron + React로 만든 크로스플랫폼(Mac/Windows/Linux) 앱. OpenAI·Google·자체 호스팅 OpenAI 호환 엔드포인트(Ollama·vLLM·LM Studio 등 `custom`으로 통합)와 연결해 챗봇과 대화하거나 AI에게 문서 수정을 직접 맡길 수 있습니다 (Anthropic 어댑터는 키 결정 대기 중). 편집 UI 는 벤더링한 rhwp-studio 에디터를 `ahwp-studio://` 프로토콜로 임베드한 iframe 이 담당합니다.

## 핵심 기능

- **HWP/HWPX 뷰어·에디터** — 벤더링한 rhwp-studio 에디터(`vendor/rhwp/rhwp-studio`)를 `ahwp-studio://` 커스텀 프로토콜로 임베드한 iframe 으로 한글 문서를 보고 수정 (오프라인 동작, 외부 iframe 의존 없음 — 로컬 번들만 로드)
- **두 가지 시작 방식**
...
- **멀티 AI 백엔드** — OpenAI · Google · `custom` (OpenAI 호환 엔드포인트 — 자체 호스팅 Ollama / vLLM / LM Studio / on-prem 게이트웨이 모두 같은 슬롯). Anthropic 은 ProviderId 에 예약돼 있으나 어댑터 미구현 (키 결정 대기)
- **세 가지 편집 경로**
  - **직접 편집**: 위의 풀 편집 기능을 rhwp-studio iframe 에디터로 실행
...
- **HWP / HWPX** — `@rhwp/core` (Rust+WASM) 기반 (iframe 내부 rhwp-studio 가 사용). 저장은 HWP/CFB로 통일 (HWPX 라운드트립 이미지 손실 회피, KNOWN_ISSUES L-001)
...
| Anthropic            | `ANTHROPIC_API_KEY` (어댑터 미구현 — 키 결정 대기, 현재 호출 시 "not implemented yet")                                                            |
| Google               | `GOOGLE_API_KEY`                                                                                                                                  |
| 커스텀 (OpenAI 호환) | Base URL + 키. 자체 호스팅 Ollama (`http://localhost:11434/v1`) · vLLM · LM Studio · on-prem 게이트웨이 (NIM 셀프호스트 포함) 모두 한 슬롯에 통합 |
...
| Anthropic            | — (어댑터 미구현) | Messages API의 `web_search` server tool 지원 예정 (어댑터 구현 후)                                       |
...
| 상태      | React hooks (per-feature) + 명시적 ref 핸들 (RhwpEditor / bridge)                                                      |
| HWP 코어  | `@rhwp/core` 0.7.x (Rust+WASM) — iframe 내부 rhwp-studio 가 사용, host 는 `ahwp-studio://` 프로토콜로 임베드           |
| 저장소    | better-sqlite3 (chat history), `safeStorage` 암호화 (API 키), JSON (session/recent/model-cache)                        |
| AI 어댑터 | 모두 직접 `fetch` (provider SDK 미사용) — OpenAI · Google · `custom` (OpenAI 호환 엔드포인트). Anthropic 어댑터 미구현 |
...
│   ├── ipc/
│   │   ├── file.ts        file:new/open/read/save/save-as/export + 버전 스냅샷 (userData/versions/)
...
│   ├── ai/
│   │   ├── providers/     openai / google / custom / fake (env-gated) — 모두 직접 fetch, provider SDK 미사용 (anthropic 어댑터 미구현)
│   │   └── registry.ts    Provider 인터페이스 + 어댑터 등록 (openai / google / custom)
│   ├── hwp/
...
├── src/                   렌더러 (React 19)
│   ├── App.tsx · main.tsx
│   ├── app/               AppShell · TitleBar · TabBar · WelcomePane · ThemeProvider · ThemeToggle
│   ├── features/
│   │   ├── files/         FolderTree + SearchPanel
│   │   ├── rhwp-studio/   RhwpEditor (vendored rhwp-studio iframe 호스트, ahwp-studio://) + bridge-ir-helper
│   │   ├── chat/          ChatPanel (Plan/Agent + Diff Viewer + 히스토리) + hooks
│   │   ├── settings/      SettingsDialog (4탭 — 일반/AI/단축키/정보)
│   │   ├── cmdk/          CommandPalette (⌘K) + ShortcutsDialog (⌘/)
│   │   └── updater/       UpdateBanner (auto-updater UI)
